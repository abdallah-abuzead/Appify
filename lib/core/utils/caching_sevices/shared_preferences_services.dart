...
abstract class SharedPreferencesServices {
  Future saveData({required String key, required dynamic value, required DataType dataType});
...
class SharedPreferencesServicesImpl implements SharedPreferencesServices {
  ...
  @override
  Future saveData({required String key, required dynamic value, required DataType dataType}) async {
    await _getSharedPrefsMethod(restoring: false, dataType: dataType)(key, value);
  }

  @override
  Future<dynamic> getData({required String key, required DataType dataType}) async {
    return await _getSharedPrefsMethod(restoring: true, dataType: dataType)(key);
  }
  ...
  @override
  Future<bool> clearKey({required String key}) async => await prefs.remove(key);

  dynamic _getSharedPrefsMethod({required bool restoring, required DataType dataType}) {
    switch (dataType) {
      case DataType.bool:
        {
